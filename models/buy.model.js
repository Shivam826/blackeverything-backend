...
class BuyModel {
    ...
    getTokenPurchase = async (id) => {
        let sql = `SELECT t.token, t.amount,t.stripe_payement_id,t.receipt_url, t.transactionHash,t.fee,t.status, DATE_FORMAT(t.created_at, '%Y-%m-%d %H:%i:%s') as created_at, p.phase FROM transactions as t LEFT JOIN phase as p ON t.phase = p.id WHERE t.user_id = '${id}' ORDER BY t.id DESC `;
        const [result, fields] = await promisePool.query(sql);
