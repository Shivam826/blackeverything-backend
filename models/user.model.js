...
class UserModel {
    ...
    getkyc = async () => {
        let sql = `SELECT id, first_name, email, kyc_document, kyc_proof_of_address, kyc_document_image, kyc_document_type , kyc_document_type2, kyc_document_image2, kyc_approval, kyc_document2, user_photo, DATE_FORMAT(created_at, '%Y-%m-%d %H:%i:%s') as created_at FROM registration  WHERE kyc_document !='' ORDER BY kyc_approval ASC`;
        const [result, fields] = await promisePool.query(sql);
