...
getUsersListFilter = async (data) => {
  ...
  getTransactionHistory = async (data) => {
    ...
    if(type==3){
      var sql = `SELECT tr.*,r.email, p.phase as phase_name,getTotalSale()  as totalSale FROM transactions as tr inner join registration as r on r.id=tr.user_id LEFT JOIN phase as p ON p.id=tr.phase  WHERE  ${whereUserId} ORDER BY tr.id DESC`;
    }
